...
HF / AlphaXiv的投票反映的是「读论文的人怎么看」，GitHub Stars反映的是「写代码、做复现的人怎么看」。两者关注的不是同一批人，也不一定是同一件事。

| 信号维度 | 主要反映 | 典型行为 | 局限 |
|----------|----------|----------|------|
| `upvotes`（HF） | 论文阅读社区的显式认可 | 浏览 Daily Papers 后投票 | 偏「读」与「讨论」，不一定有代码产出 |
| `likes`（AlphaXiv） | 研究社区的热度与传播 | 在 AlphaXiv 上点赞、追踪 | 与 HF 有重叠，但仍属论文平台内信号 |
| `github_stars` | 工程落地与开源影响力 | clone、star、fork、集成进项目 | 无仓库的论文 star 为 0，不能单独代表学术质量 |

我们对**2026-06**榜单做了Pearson相关分析（详见 [`2026_06_统计.txt`](top_papers/2026_06_统计.txt)）：
